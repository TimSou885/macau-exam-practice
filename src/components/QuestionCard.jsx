function OptionButton({ option, selected, correct, showResult, disabled, onSelect }) {
  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border-2 p-4 text-left transition focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:pointer-events-none min-h-[44px] ${bg}`}
    >
...
export default function QuestionCard({ question, selectedKey, onSelect, showResult, disabled }) {
  ...
  return (
    <article className="rounded-2xl border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-800/50">
      ...
      <ul className="mt-6 flex flex-col gap-3" role="listbox" aria-label="選項">
      ...
      {showResult && question.explanation && (
        <div className="mt-6 rounded-lg bg-stone-100 p-4 dark:bg-stone-700/50">
